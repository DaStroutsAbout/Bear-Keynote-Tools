...
1. Clone or download this repository — `Keynote Tools.applescript` lives on the Desktop
...
4. The first time you run Program 4, macOS will prompt for **Accessibility permission** — go to System Settings → Privacy & Security → Accessibility and enable Script Editor
...
**Wide photos (434×267):** when a wide photo is present, the body text box is always placed full-width (700px) at x=34 in the next row below the wide photo, with an additional 20pt of spacing to account for the image caption label. The wide photo's row is detected by its y position after layout.
